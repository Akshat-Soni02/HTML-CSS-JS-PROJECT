...
const updateScoreBoard = () => {
    ...
    p1Ats.ariaLabel = `Computer Player has ${Game.getP1AllTime()} all time wins.`;
...
const computerAnimationSequence = (playerChoice) => {
    ...
    setTimeout(() => computerChoiceAnimation("cp_scissor",3) , interval+= 500);
...
const updateAriaResult = (result, winner) => {
    ...
    const winMessage = winner === "player" ? "Congratulation, you are the winner."
    : winner === "computer" ?
...
const resetBoard = () => {
    ...
    cpSquares.forEach((el) => {
        ...
        if(el.id === "cp_scissor") createGameImage("scissors",el);
    });
